  ...
  try {
    ...
    if (voucher.statut === 'utilise' && voucher.activated_at) {
      ...
      if (stillValid) {
        ...
        await query(
          "UPDATE sessions_actives SET statut='expiree', updated_at=NOW() WHERE voucher_id=?",
          [voucher.id]
        );
        return res.status(400).json({
          success: false,
...
async function _createSession(voucher, mac, ip, borneId) {
  ...
  await query(
    "UPDATE sessions_actives SET statut='terminee', updated_at=NOW() WHERE mac_address=? AND statut='active'",
    [mac]
  );
